...
//               Stimuli/Timelines                                    //
////////////////////////////////////////////////////////////////////////
function stimuli_factory(items_animals, items_toys) {
  let stimuli = [];

  // Animals and toys need to be the same lenght
  items_animals = shuffle(items_animals);
  items_toys = shuffle(items_toys);

  animal_side = shuffle(repeatArray(['left', 'right'], items_animals.length / 2));
  probe_prefix = shuffle(repeatArray(['jetzt', 'nicht'], items_animals.length / 2));
  probe = shuffle(repeatArray(['Tiere', 'Spielzeug'], items_animals.length / 2));
  for (var idx = 0; idx < items_animals.length; idx++) {
    let stimulus = {};
    if (animal_side[idx] === 'right') {
      stimulus.right = items_animals[idx].img;
      stimulus.left = items_toys[idx].img;
    } else if (animal_side[idx] === 'left') {
      stimulus.right = items_toys[idx].img;
      stimulus.left = items_animals[idx].img;
    }
    stimulus.probe = probe_prefix[idx] + ' ' + probe[idx];
    if (
      probe_prefix[idx] === 'jetzt' && probe[idx] === 'Tiere' ||
      probe_prefix[idx] === 'nicht' && probe[idx] === 'Spielzeug'
    ) {
      stimulus.correct_side = animal_side[idx];
    } else if (
      probe_prefix[idx] === 'nicht' && probe[idx] === 'Tiere' ||
      probe_prefix[idx] === 'jetzt' && probe[idx] === 'Spielzeug'
    ) {
      stimulus.correct_side = animal_side[idx] === 'right' ? 'left' : 'right';
    }
...
const exp_stimuli = stimuli_factory(items_animals, items_toys);
