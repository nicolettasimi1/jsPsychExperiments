...
function codeTrial() {
    ...
    let errorCode;
    // Movement too fast
    if (dat.time[idx] < prms.fixDur) {
        errorCode = 2;
    // Wrong side
    } else if (dat.correct_side !== dat.end_loc) {
        errorCode = 1;
    // Correct
    } else {
        errorCode = 0;
...
const task_instructions = {
    ...
    "<H3 style = 'text-align: left;'> Danach erscheint eine Folge von Wörtern auf dem Bildschirm.    </H3> <br>" +
    "<H3 style = 'text-align: left;'> Ihre Aufgabe ist es, das Bild auszuwählen, das am besten zu dem   </H3>" +
    "<H3 style = 'text-align: left;'> Wort passt und den Mauszeiger in das zugehörige Quadrat zu bewegen.    </H3>" +
    "<H3 style = 'text-align: left;'> In einigen Durchgängen ist dies einfach. In anderen Durchgängen  </H3>" +
    "<H3 style = 'text-align: left;'> gibt es allerdings keine direkte Übereinstimmung zwischen dem Wort  </H3>" +
    "<H3 style = 'text-align: left;'> und den zwei Bildern. In diesem Fall, wählen Sie einfach das Bild  </H3>" +
    "<H3 style = 'text-align: left;'> bei dem Sie intuitiv denken, dass es am besten zu dem Wort passt.   </H3>" +
    "<H3 style = 'text-align: left;'> Bitte reagieren Sie so schnell und so korrekt wie möglich.    </H3>" +
    "<H3 style = 'text-align: left;'> Zuerst folgt ein Übungsblock, indem Sie zusätzlich Feedback zu Ihren Antworten erhalten.    </H3>" +
    "<H3 style = 'text-align: left;'> Im ersten Teil des Übungsblocks werden noch keine Bilder zu sehen sein.</H3>" +
...
function stimuli_factory(items) {
    ...
    for (let [idx, item] of items_shuffled.entries()) {
        ...
        if ('correct_side' in item) {
            stimulus.correct_side = item.correct_side;
        }
...
const trial_stimulus = {
    ...
    data: {
        ...
        target_1_relation: jsPsych.timelineVariable('target_1_relation'),
        target_2_relation: jsPsych.timelineVariable('target_2_relation'),
    },
    on_finish: function () {
        codeTrial();
    },
...
const exp_timeline = {
    ...
    // timeline: [trial_stimulus, trial_feedback, iti],
    // Removing the 'trial_feedback' stop showing the feedback to the partecipant
    // Is this corerect? Does 'trial_feedback' set iportant informations? 'trial_duration' is set here.
